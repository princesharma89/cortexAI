...
function Artifact() {
  ...
  const file = latestArtifact?.files?.[activeFile];
  const htmlFile = latestArtifact?.files?.find((f) => f.name === "index.html");
  const cssFile = latestArtifact?.files?.find((f) => f.name === "style.css");
  const jsFile = latestArtifact?.files?.find((f) => f.name === "script.js");
...
<script>
  ...
  const PanelContent = ({ onClose }) => {
    const isExpanded = !collapsed || Boolean(onClose);

    return (
      <>
        {isExpanded ? (
          <div className="flex flex-col h-full w-full lg:w-[400px]">
            <div className="h-14 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0">
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div className="text-[13px] font-medium text-slate-200 truncate">
                  {latestArtifact?.title || latestArtifact?.type || "Artifacts"}
                </div>
            ...
            {tab === "code" && (
              <div className="flex h-auto border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0">
                {latestArtifact?.files?.map((f, index) => (
                  <button
                    key={index}
          ...
          <div className="flex flex-col items-center h-full w-[48px] py-4 gap-3 shrink-0">
            <button
              className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0"
