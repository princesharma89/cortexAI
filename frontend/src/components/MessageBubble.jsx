...
function MessageBubble({ role, content, images = [] }) {
  ...
  return (
    <>
          ...
          <Markdown
            ...
            components={{
              ...
              code: ({ className, children }) => {
                ...
                if (!className) {
                  return (
                    <code className='px-1.5 py-0.5 rounded bg-white/10 text-indigo-200 text-sm'>
                      {value}
                    </code>
                  )
                }
      ...
      {/* Lightbox Modal Rendered Outside Outer Flex Wrapper */}
      {lightBox && (
        <div className='fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6'>
          <button
            className='absolute top-5 right-5 text-white/80 hover:text-white bg-white/10 rounded-full p-2 transition'
